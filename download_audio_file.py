...
is_recording_busy = False # Safety flag to prevent overlapping recordings
running_flag = True
clicked_point, measure_text = None, ""
global_depth_map, global_raw_warped = None, None

# --- 3. Audio Recording Settings (USB Adapter) ---
DEVICE_ID = 1    # AB13X USB Audio
FREQ = 48000     
SECONDS = 5

def record_audio_task(scene_folder):
    """Background task to record audio and convert to MP3."""
    global is_recording_busy
    is_recording_busy = True
    filename = os.path.join(scene_folder, "voice_note.mp3")
    temp_wav = f"temp_{int(time.time())}.wav"
    
    try:
        logger.info(f"Recording audio to {filename}...")
        play_audio_blocking("recording_started.mp3")
        
        # Capture from USB
        recording = sd.rec(int(SECONDS * FREQ), samplerate=FREQ, channels=2, device=DEVICE_ID)
        sd.wait()
        
        # Export as MP3
        write(temp_wav, FREQ, recording)
        ...
        is_recording_busy = False

def save_scene_worker(s_num, l_img, r_img, d_gray, d_raw, t_warp, t_raw, r_viz):
    """Saves all visual data and then triggers the audio recording."""
    base = os.path.join(ROOT_IMG_DIR, f"Scene_{s_num}")
    for s in ["left_cam", "right_cam", "depth", "thermal", "annotated"]:
        os.makedirs(os.path.join(base, s), exist_ok=True)
    
    # Save Images
    cv2.imwrite(os.path.join(base, "left_cam/left.png"), l_img)
    cv2.imwrite(os.path.join(base, "right_cam/right.png"), r_img)
    cv2.imwrite(os.path.join(base, "depth/depth.png"), d_gray)
    np.save(os.path.join(base, "depth/depth.npy"), d_raw)
    cv2.imwrite(os.path.join(base, "thermal/thermal.png"), t_warp)
    if t_raw is not None:
        np.save(os.path.join(base, "thermal/thermal.npy"), (t_raw / 100.0) - 273.15)
    cv2.imwrite(os.path.join(base, "annotated/annotated.png"), r_viz)
    
    with open(os.path.join(base, "metadata.json"), "w") as f:
        json.dump({"Q": Q.tolist(), "scene": s_num, "timestamp": time.time()}, f, indent=4)
    
    logger.info(f"Visual data for Scene {s_num} saved. Starting audio recording...")
    record_audio_task(base)

# --- 4. Hardware Button ---
def request_capture():
    global trigger_capture_flag
    if not is_recording_busy:
        trigger_capture_flag = True
    else:
        logger.warning("Still recording previous scene. Please wait.")
...
thermal_camera_path = '/dev/v4l/by-id/usb-GroupGets_PureThermal__fw:v1.3.0__001b002b-5102-3133-3332-373300000000-video-index0'
...
def nothing(x): pass
cv2.createTrackbar("ROI X", "Fine Tune", 143, W, nothing); cv2.createTrackbar("ROI Y", "Fine Tune", 18, H, nothing)
cv2.createTrackbar("ROI W", "Fine Tune", 473, W, nothing); cv2.createTrackbar("ROI H", "Fine Tune", 583, H, nothing)
cv2.createTrackbar("Parallax", "Fine Tune", 100, 200, nothing)
cv2.createTrackbar("Hot Thresh", "Fine Tune", 190, 255, nothing); cv2.createTrackbar("Cold Thresh", "Fine Tune", 80, 255, nothing)
...
try:
    while running_flag:
        ...
        if trigger_capture_flag:
            ...
            # Snap copies of everything so the thread works with the current frame
            t_raw_snap = global_raw_warped.copy() if global_raw_warped is not None else None
            
            args = (
                scene_num, rectL.copy(), rectR.copy(), gray_d.copy(), 
                depth_map.copy(), thermal_warped.copy(), t_raw_snap, rgb_viz.copy()
            )
            
            # Start background thread
            threading.Thread(target=save_scene_worker, args=args).start()
